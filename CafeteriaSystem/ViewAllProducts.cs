    ...
    public partial class ViewAllProducts : Form
    {
        ...
        private void ProductCategoryComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ProductCategoryComboBox.SelectedIndex == 0)
            {
                ...
                {
                    ProductsGridView.Rows.Add(ProductDetail.ID, ProductDetail.Name, ProductDetail.Price, ProductDetail.Category, ProductDetail.Unit, ProductDetail.Quantity);
                }
            }
            else if (ProductCategoryComboBox.SelectedIndex > 0)
            {
                ...
                {
                    ProductsGridView.Rows.Add(ProductDetail.ID, ProductDetail.Name, ProductDetail.Price, CategoryName, ProductDetail.Unit, ProductDetail.Picture);
                }
            }
        }
        ...
        private void ProductsGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                else if (ProductsGridView.Columns[e.ColumnIndex].Name == "EditProductColumn")
                {
                    ...
                    string ProductUnit = ProductsGridView.Rows[e.RowIndex].Cells["ProductUnitColumn"].Value.ToString();

                    string ProductQuantity = ProductsGridView.Rows[e.RowIndex].Cells["ProductQuantity"].Value.ToString();

                    UpdateProducts UpdateProductForm = new UpdateProducts(ProductID, ProductName, ProductPrice, ProductCategory, ProductUnit, ProductQuantity);
                    ...
                    if (UpdateProductForm.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        ProductsGridView.Rows[e.RowIndex].Cells["ProductUnitColumn"].Value = UpdatedProductDetail.Unit;
                        ProductsGridView.Rows[e.RowIndex].Cells["ProductQuantity"].Value = UpdatedProductDetail.Quantity;
